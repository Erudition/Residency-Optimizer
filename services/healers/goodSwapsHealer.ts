...
export const goodSwapsHealer: HealerSolver = {
    name: "Good Swaps Healer (4 & 2 Block)",
    solve: async (residents: Resident[], existingSchedule: ScheduleGrid, attemptIndex: number = 0, priorRequirementCounts?: Record<string, Record<string, number>>, cohortAssignments?: Record<string, number>, onProgress?: (step: number, maxSteps: number, currentPenalty: number) => void): Promise<ScheduleGrid> => {
        ...
        console.log(`[Good Swaps] Initial violations: ${currentTotalViolations} (Req: ${currentReqViolations}, Staffing: ${currentStaffingGaps})`);
        ...
        while (improved && iter < maxIter) {
            ...
            console.log(`[Good Swaps] Iteration ${iter} finished. Beneficial Swaps -> 4-Block: ${swaps4Block}, 2-Block: ${swaps2Block}. Violations: ${currentTotalViolations} (Req: ${currentReqViolations}, Staff: ${currentStaffingGaps})`);
        }

        console.log(`[Good Swaps] Final violations: ${currentTotalViolations} (Req: ${currentReqViolations}, Staffing: ${currentStaffingGaps})`);
        return schedule;
    }
};