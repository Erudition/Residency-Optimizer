...
## Phase 4: Phase 2 Healer Service Implementation (COMPLETED)
...
- [x] **Task 4.3**: Hard Constraints: Ensure the healer never introduces staffing violations or touches clinic weeks.
- [x] **Task 4.4**: Locked Preservation: Ensure healer respects all `locked` cells and the 4-week continuity prefix.
- [x] **Task 4.5**: **Verification Gate**: `npx tsc --noEmit` + Convergence tests (Staffing > Requirements).

## Phase 5: UI Integration
*Goal: Expose healer status and manual healing capability to the user.*

- [x] **Task 5.1**: `GenerationDashboard`: Add Phase 2 (Healer) progress tracking.
- [x] **Task 5.2**: `ScheduleTable`: Add "Heal Schedule" button for manual violation resolution.
- [x] **Task 5.3**: `GlobalOptimizer`: Wire Healer to run automatically after a Phase 1 winner is selected.
