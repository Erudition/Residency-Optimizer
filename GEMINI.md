...
*   **Property Mapping** — Two requirement builder functions exist in `reqBuilder.ts`:
    *   `buildLevelRequirements`: Legacy function for generators without soft-goal awareness. Maps `pgy*Ideal` → `minWeeks` so generators aim for ideals as if they were hard minimums (the only way to make them try at all).
    *   `buildEnrichedLevelRequirements`: Returns `EnrichedRequirement` with both `idealWeeks` (soft goal per PGY year, from backend `pgy*Ideal`) and `hardMinWeeks` (hard floor per PGY year, pro-rated from graduation `minimum` using ideal distribution as weights). Used by the CSP generator and available for RequirementsEngine reporting.
...
*   Browser launching: ALWAYS check for an existing chromium tab before launching a new browser instance. If I say "check the open tab" or "see the browser console errors", do not use "navigate" to refresh or open a new page.
...
*   **Healer Is Manual-Only**: The Healer is never automatically applied after generation. It is a manual step invoked by the user from the Healer Panel. Generators must produce the best possible schedule without relying on post-hoc healing. (1-block swaps in particular created "salad" schedules that disrupted block continuity.)
*   **Year-Boundary Truncated Blocks**: At the start of each academic year, the weeks before the first clinic week are a truncated inpatient window (since cohort assignments may have changed from the prior year). These truncated blocks should still be filled with educational rotations where possible, not defaulted to electives. The standard block-duration preference (e.g., 4-week contiguous blocks) is relaxed for these year-boundary windows.
